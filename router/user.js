...
// http://localhost:4000/api/v1/users/register Register new data for user in Database
...
router.post(`/login`, async (req, res) => {
  ...
  if (email && bcrypt.compareSync(req.body.passwordHash, email.passwordHash)) {
    ...
    return res.status(202).send({ email: email.email, token: token });
  } else {
...
//http://localhost:4000/api/v1/users/get/count this api using for getting exact no of count Users currently not working unable to fix
